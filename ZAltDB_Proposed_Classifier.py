...
classLabels = ['Asphalt', 'Meadows', 'Gravel', 'Trees' ,' Painted metal sheets', 
                ' Bare Soil', 'Bitumen', 'Self-Blocking Bricks', 'Shadows']


HSI = sio.loadmat('..\..\DB\PaviaU\PaviaU.mat')
gt = labels = sio.loadmat('..\..\DB\PaviaU\PaviaU_gt.mat')
...
# split trai - test
X_train, X_test, y_train, y_test = am.trainTestSplit(HSIwPCA, gt, 0.7)
#X_train.shape, X_test.shape, y_train.shape, y_test.shape

# split train - val
X_train, X_val, y_train, y_val = am.trainTestSplit(X_train, y_train, 0.3)
...
# Train and fit
start = time.time()
resTrain = model.fit(x = X_train, y = y_train, batch_size = 256, epochs = 20, validation_data = (X_val, y_val))
end = time.time()
...
for test in range(1):
    ...
    tau = np.ones((outputs.shape[0], outputs.shape[1], no_of_classes.size))
    
    eta = np.ones((outputs.shape[0], outputs.shape[1], no_of_classes.size))
    ...
    p = np.ones((aco_map.shape[0], aco_map.shape[1], no_of_classes.size))/10
    
    # x, y = pixel coordinates, k = edge/class
    for ep in range(epochs):
        for i in range(aco_map.shape[0] - 1):                    
            for j in range(aco_map.shape[1] - 1):
                f = np.zeros((no_of_classes.size))
                for k in range(no_of_classes.size - 1):                
    ...
    gt = gt.astype(int)
    scoreclassf,  scoreToAppend = am.myScoreReloaded(gt, clmap, aco_map, 610*340*0.3, 610*340)
    score[test][0] = alpha
    score[test][1] = beta
    score[test][2] = rho
